...
*&---------------------------------------------------------------------*
*& FS-MM-301MOV-001 : BAdI implementation of MB_DOCUMENT_BADI.
*&
...
*&   Method    : MB_DOCUMENT_BEFORE_UPDATE  (runs in the update task)
*&---------------------------------------------------------------------*
...
CLASS zcl_mm_301_gr_trigger IMPLEMENTATION.
  ...
  METHOD if_ex_mb_document_badi~mb_document_before_update.
    " Importing (from interface): XMKPF TYPE mkpf, XMSEG TYPE <mseg table>.
    LOOP AT xmseg ASSIGNING FIELD-SYMBOL(<seg>).
      ...
      IF ( ls_ctrl-valid_from IS NOT INITIAL AND xmkpf-budat < ls_ctrl-valid_from )
      OR ( ls_ctrl-valid_to   IS NOT INITIAL AND xmkpf-budat > ls_ctrl-valid_to ).
        CONTINUE.
      ENDIF.
      ...
      CALL FUNCTION 'Z_MM_301_POST_TRANSFER'
        IN BACKGROUND TASK
        EXPORTING
          iv_mblnr    = xmkpf-mblnr
          iv_mjahr    = xmkpf-mjahr
          iv_zeile    = <seg>-zeile
